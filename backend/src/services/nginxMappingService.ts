...
export async function updateSubdomainMapping(subdomain: string, port: number) {
    console.log(`📝 Adding mapping: ${subdomain}.localhost -> port ${port}`)
    ...
    try {
        ...
        console.log(`✅ Nginx reloaded - ${subdomain}.localhost is now LIVE!`)
    } catch (error: any) {
...
export async function removeSubdomainMapping(subdomain: string) {
    console.log(`🗑️ Removing mapping for ${subdomain}.localhost`)
    ...
    try {
        ...
        await execAsync('sudo systemctl reload nginx')
        console.log(`✅ Mapping removed for ${subdomain}.localhost`)
    } catch (error) {
